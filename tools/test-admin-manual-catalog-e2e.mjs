...
vm.runInContext([
  ...
  `
    var TEST_DB = {
      ...
      materiais:[], plano_itens:[], usuarios:[], sessoes:[], audit_log:[], ordens_servico:[], os_acoes:[],
...
assert(evaluate(`rows_('audit_log').length`) === 2, 'cadastros confirmados não foram auditados')
assert(lockAcquisitions === 2 && lockReleases === 2, 'lock não envolveu exatamente as gravações válidas')

console.log('CADASTRO MANUAL E2E EM MEMÓRIA APROVADO')
console.log('Dropdowns são reforçados por referências validadas no backend')
console.log('Planos permanecem inativos, validação não pode ser burlada e gravações são auditadas')

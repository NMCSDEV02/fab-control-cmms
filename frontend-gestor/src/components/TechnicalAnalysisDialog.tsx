...
function defaultCause(occurrence: GestorOccurrence): string {
  const text = `${occurrence.titulo ?? ''} ${occurrence.descricao ?? ''}`
    .toLocaleLowerCase('pt-BR')
  if (text.includes('ruído') || text.includes('vibra')) {
    return 'Desgaste, desalinhamento ou fixação inadequada'
  }
  if (text.includes('temperatura') || text.includes('quente') || text.includes('aquec')) {
...
function defaultSteps(occurrence: GestorOccurrence): GestorTechnicalStep[] {
  ...
  return [
    ...
    },
    {
      ...
      descricao: 'Verificar o ponto relatado, registrar parâmetros e coletar evidências antes da intervenção.',
    },
    {
      ordem: 3,
      titulo: 'Corrigir ou estabilizar',
      descricao: 'Executar somente a atividade autorizada e registrar qualquer desvio de escopo.',
    },
    {
      ordem: 4,
      titulo: 'Testar e liberar',
      descricao: 'Confirmar o resultado, anexar evidências e registrar a condição operacional final.',
    },
  ]
}

function defaultBrief(occurrence: GestorOccurrence): GestorTechnicalBrief {
  const source = `${occurrence.titulo ?? ''} ${occurrence.descricao ?? ''}`
    .toLocaleLowerCase('pt-BR')
  ...
  return {
    ...
    nrs,
    ferramentas: [
      { tipo: 'INSPECAO', nome: 'Instrumento de medição compatível com o diagnóstico' },
...
export function TechnicalAnalysisDialog({
  ...
  const [probableCause, setProbableCause] = useState(initialBrief.causa_provavel)
  const [recommendation, setRecommendation] = useState(
    'Realizar inspeção orientada, tratar a causa confirmada e validar o retorno operacional.',
  )
  const [expectedResult, setExpectedResult] = useState(initialBrief.resultado_esperado)
  const [acceptance, setAcceptance] = useState(initialBrief.criterio_aceite)
  const [steps, setSteps] = useState<GestorTechnicalStep[]>(initialBrief.etapas)
    ...
    <div className="review-overlay" role="presentation">
      ...
      >
        <header className="review-dialog__header">
          <div>
            <span className="eyebrow">ANÁLISE ASSISTIDA · {occurrence.ativo_id || 'SEM ATIVO'}</span>
            <h2 id="technical-analysis-title">Transformar ocorrência em demanda</h2>
        ...
        <nav className="manager-analysis-stages" aria-label="Etapas da análise">
          ...
          ] as Array<[AnalysisStage, string, string]>).map(([id, number, label]) => (
            <button
            ...
            <section className="manager-analysis-section">
              ...
              <div className="manager-analysis-fields">
                ...
                <label>
                  ...
                  >
                    <option value={initialBrief.causa_provavel}>{initialBrief.causa_provavel}</option>
                    ...
                    <option value="A confirmar por inspeção técnica no equipamento">A confirmar na inspeção</option>
                  </select>
                </label>
                <label>
          ...
          {stage === 'plan' ? (
            <section className="manager-analysis-section">
              ...
              <div className="manager-analysis-demands">
                ...
                >
                  <CheckIcon />
                  <span><strong>Modelo de checklist</strong><small>O Admin recebe a estrutura pronta para completar.</small></span>
                </button>
                <button
                ...
                >
                  <WrenchIcon />
                  <span><strong>Ordem de serviço</strong><small>Solicita uma intervenção administrativa.</small></span>
                </button>
              </div>
            </section>
          ) : null}
          ...
          {stage === 'execution' ? (
            <section className="manager-analysis-section">
              <div className="manager-briefing-summary">
                <article>
                  <header><ShieldIcon /><strong>Segurança</strong><span>{initialBrief.nrs.join(' · ')}</span></header>
                  <ul>
                    {initialBrief.seguranca.map((item) => <li key={item}>{item}</li>)}
                  </ul>
                </article>
                <article>
                  <header>
                    ...
                    <button type="button" onClick={() => setEditSteps((current) => !current)}>
                        ...
                        {editSteps ? (
                          <span>
                            ...
                            <textarea
                              ...
                              onChange={(event) => updateStep(index, { descricao: event.target.value })}
                              aria-label={`Descrição da etapa ${index + 1}`}
                            />
                          </span>
                        ) : (
                          <span><strong>{step.titulo}</strong><p>{step.descricao}</p></span>
                        )}
                    ...
                    <button
                      ...
                      onClick={() => setSteps((current) => [
                        ...current,
                        {
                          ...
                          titulo: 'Nova etapa',
                          descricao: 'Descreva a atividade e a condição esperada.',
                        },
                      ])}
                    >
                      Adicionar etapa
          ...
          <div>
            {stage !== 'context' ? (
              <button
                className="secondary-button"
                type="button"
                onClick={() => setStage(stage === 'execution' ? 'plan' : 'context')}
              >
                Voltar
              </button>
            ) : null}
            {stage === 'context' ? (
              <button className="primary-button" type="button" onClick={() => goTo('plan')}>
                Continuar <ChevronRightIcon />
              </button>
            ) : null}
            {stage === 'plan' ? (
              <button className="primary-button" type="button" onClick={() => goTo('execution')}>

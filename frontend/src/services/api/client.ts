...
async function executeAppsScriptCall<T>(
  ...
  if (!response.ok) {
    throw new ApiRequestError(`A API respondeu com HTTP ${response.status}.`, 'HTTP_ERROR', {
  ...
  try {
    ...
    throw new ApiRequestError('A API não retornou JSON válido.', 'INVALID_JSON', error)
  }
...
function operatorExecutionMode(value: unknown): string {
  return stringValue(value).toUpperCase() === 'NO_STOP' ? 'SEM_PARADA' : 'COM_PARADA'
}

function checklistItem(value: unknown, executionId: string, actionId: string): JsonRecord {
  ...
  const answered = ['ANSWERED', 'NONCOMPLIANT', 'NOT_APPLICABLE'].includes(status)
  ...
  return {
    ...
    input: {
      ...
      requer_valor: ['NUMERO', 'PARAMETRO', 'LEITURA_OPERACIONAL'].includes(responseType),
      requer_opcoes: ['OK_NOK', 'SELECAO'].includes(responseType),
      suporta_evidencia: item.evidencia_obrigatoria === true || responseType === 'EVIDENCIA',
      opcoes: asArray(item.opcoes).map(stringValue).filter(Boolean),
      unidade: stringValue(item.unidade),
      limite_min: item.minimo,
      limite_max: item.maximo,
...
function actionAvailability(action: JsonRecord): JsonRecord {
  ...
  return {
    ...
    estado: !plannedAt ? 'SEM_AGENDAMENTO' : canStart ? 'DISPONIVEL' : 'AGENDADA',
...
function operatorActionDetail(data: JsonRecord): JsonRecord {
  ...
  const sourceItems = executionId ? asArray(execution.itens) : asArray(action.checklist_itens)
  const items = sourceItems.map((item) => checklistItem(item, executionId, actionId))
  ...
  const missingEvidence = items.filter(
    (item) =>
      ...
      numberValue(item.evidencias_count) < Math.max(1, numberValue(item.evidencia_min_fotos)),
  ).length
  ...
  const inProgress = executionId.length > 0 && executionStatus === 'EM_EXECUCAO'
  ...
  return {
    ...
    execucao: executionId
      ? {
          ...
          modo_execucao_manutencao: operatorExecutionMode(execution.modo_parada),
        }
      : null,
    ...
    ui: {
      ...
      message: inProgress ? 'Execução em andamento.' : 'Revise a análise técnica antes de iniciar.',
    },
    operator_screen: {
      header: {
        acao_id: actionId,
      ...
      progress: {
        ...
        respondidos: answered,
        pendentes: pending,
        percentual: items.length ? Math.round((answered / items.length) * 100) : 0,
...
function operatorExecutionChecklist(execution: JsonRecord, actionId: string): JsonRecord {
  const executionId = stringValue(execution.id)
  const items = asArray(execution.itens).map((item) => checklistItem(item, executionId, actionId))
  const answered = items.filter((item) => item.respondido === true).length
  const pending = items.filter(
    (item) => item.obrigatorio === true && item.respondido !== true,
  ).length
...
function operatorStartResult(data: JsonRecord, actionId: string): JsonRecord {
  ...
  return {
    ...
    decisao_parada_manutencao: mode === 'SEM_PARADA' ? 'SEM_PARADA' : 'PARAR_EQUIPAMENTO',
    execucao: {
      id: executionId,
      acao_id: actionId,
      operador_id: stringValue(execution.operador_id),
...
function operatorState(data: JsonRecord, actionId: string): JsonRecord {
  ...
  return {
    ok: true,
    started: executionId.length > 0 && operatorStatus(execution.status) === 'EM_EXECUCAO',
    acao_id: actionId,
...
function qrParameter(value: unknown): JsonRecord {
  ...
  const latestValue =
    numeric !== null && numeric !== undefined
      ? numeric
      : parameter.ultimo_valor_texto !== null && parameter.ultimo_valor_texto !== undefined
  ...
  return {
    ...
    parametro: stringValue(parameter.codigo || parameter.parametro || parameter.nome),
    ...
    registrado_em: stringValue(parameter.ultima_leitura_em || parameter.registrado_em),
    ...
    classificacao: stringValue(parameter.ultima_classificacao || parameter.classificacao),
  }
}
...
function qrStop(value: unknown): JsonRecord | null {
  ...
  return {
    ...
    componente_id: stringValue(stop.componente_id || stop.component_id) || undefined,
    ...
    manutencao_iniciada_em: stringValue(stop.manutencao_iniciada_em || stop.maintenance_started_at),
    ...
    tempo_parada_segundos: numberValue(stop.tempo_parada_segundos || stop.downtime_seconds),
    tempo_espera_manutencao_segundos: numberValue(
      stop.tempo_espera_manutencao_segundos || stop.maintenance_wait_seconds,
    ),
    tempo_execucao_segundos: numberValue(stop.tempo_execucao_segundos || stop.execution_seconds),
    tempo_retorno_operacional_segundos: numberValue(
      stop.tempo_retorno_operacional_segundos || stop.operational_return_seconds,
    ),
    elapsed_seconds: numberValue(stop.elapsed_seconds || stop.duracao_atual_segundos),
    motivo_parada: stringValue(stop.motivo_parada || stop.motivo || stop.reason),
    categoria_retorno: stringValue(stop.categoria_retorno || stop.return_category),
...
function qrOccurrence(value: unknown): JsonRecord {
  ...
  return {
    ...
    componente_id: stringValue(occurrence.componente_id || occurrence.component_id) || undefined,
    ...
    os_id: stringValue(occurrence.os_id || occurrence.work_order_id) || undefined,
    analise_tecnica_id:
      stringValue(occurrence.analise_tecnica_id || occurrence.technical_analysis_id) || undefined,
    tratamento_status: stringValue(occurrence.tratamento_status || occurrence.treatment_status),
    criado_em: stringValue(occurrence.criado_em || occurrence.criada_em || occurrence.created_at),
    atualizado_em: stringValue(
      occurrence.atualizado_em || occurrence.atualizada_em || occurrence.updated_at,
    ),
  }
}
      ...
      ? {
          ...
          modo:
            stringValue(asset.horimetro_modo).toUpperCase() === 'TELEMETRY'
              ? 'TELEMETRIA'
              : 'MANUAL',
          automatico: stringValue(asset.horimetro_modo).toUpperCase() === 'TELEMETRY',
...
function nodeActionRequest(
  ...
  switch (action) {
    ...
    case 'operador.iniciar_acao': {
      ...
      const noStop = stringValue(payload.decisao_parada_manutencao) === 'SEM_PARADA'
      return {
        method: 'POST',
    ...
    case 'operador.historico_qr': {
      ...
      parameters.set('limite', String(Math.min(100, Math.max(1, numberValue(payload.limit) || 4))))
      return {
        method: 'GET',
    ...
    case 'operador.iniciar_parada':
      return {
        ...
        body: {
          ...
          motivo: stringValue(payload.motivo_parada) || 'Parada registrada pelo Operador.',
    ...
    case 'operador.finalizar_parada': {
      ...
      return {
        ...
        body: {
          status: 'COMPLETED',
          categoria_retorno: stringValue(payload.categoria_retorno) || 'RETORNO_NORMAL',
          justificativa_divergencia: stringValue(payload.justificativa_divergencia) || null,
        },
        transform: (data) => ({
          closed: true,
          ...
          metricas: {
            ...
            tempo_espera_manutencao_segundos: numberValue(data.maintenance_wait_seconds),
            tempo_execucao_segundos: numberValue(data.execution_seconds),
            tempo_retorno_operacional_segundos: numberValue(data.operational_return_seconds),
          },
        }),
      }
    }
    case 'operador.registrar_ocorrencia':
      return {
        ...
        body: {
          ativo_id: payload.ativo_id,
          componente_id: stringValue(payload.componente_id) || null,
          tipo: stringValue(payload.tipo) || stringValue(payload.alvo_ocorrencia) || 'OPERACIONAL',
